...
from (
         select  DATE_FORMAT(CONVERT_TZ(`posted-date`, '+00:00', '+1:00') , '%Y-%m-01')       as month,
                 CONVERT_TZ(TIMESTAMP(DATE_FORMAT(`posted-date`, '%Y-%m-%d %H:%i:%s.%f')), '+00:00', '+1:00')   as date_time,
         ...
         from main.de_get_v2_settlement_report_data_flat_file ff
                  left join main.de_get_flat_file_all___y_last_update_general gn
         ...
         and `posted-date`!=''
         and  CONVERT_TZ(`posted-date`, '+00:00', '+1:00')>= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 2 MONTH), '%Y-%m-01')

         group by  DATE_FORMAT(CONVERT_TZ(`posted-date`, '+00:00', '+1:00') , '%Y-%m-01'),
                   CONVERT_TZ(TIMESTAMP(DATE_FORMAT(`posted-date`, '%Y-%m-%d %H:%i:%s.%f')), '+00:00', '+1:00'),
                  `settlement-id`,
                  `transaction-type`,
                  `order-id`,
                  ff.sku,
